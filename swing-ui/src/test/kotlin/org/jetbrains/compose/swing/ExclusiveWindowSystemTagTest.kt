 ...
 *
 * The two are written separately - once in Kotlin, once as a literal in the build convention that splits
 * each module's tests - and nothing but this test connects them. Left unconnected they fail in the one
 * direction that reports nothing: the task filtering for a tag no test carries selects an empty suite,
 * which passes, while every test meant to run alone falls back into the parallel task and skips on a
 * window system it now shares.
 *
