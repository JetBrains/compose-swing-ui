...
import org.jetbrains.compose.swing.animation.core.KeyframesSpec.KeyframesSpecConfig
 ...
 * out of the box: for example, [Float.VectorConverter][Float.Companion.VectorConverter] and
 * [Int.VectorConverter][Int.Companion.VectorConverter]. A custom multidimensional value can be
 * animated by supplying a converter to [AnimationVector2D] (or higher), so that each dimension is
 * animated independently with separate velocity tracking. This enables multidimensional objects to
 * be animated in a true multi-dimensional way. It is particularly useful for smoothly handling
 * animation interruptions (such as when the target changes during the animation).
 */
public interface AnimationSpec<T> {
     ...
     */
    public fun <V : AnimationVector> vectorize(converter: TwoWayConverter<T, V>): VectorizedAnimationSpec<V>
}
...
public class TweenSpec<T>(
    ...
    override fun <V : AnimationVector> vectorize(converter: TwoWayConverter<T, V>): VectorizedTweenSpec<V> =
    ...
    override fun hashCode(): Int = (durationMillis * 31 + easing.hashCode()) * 31 + delay
}
...
public class SpringSpec<T>(
    ...
    override fun <V : AnimationVector> vectorize(converter: TwoWayConverter<T, V>): VectorizedSpringSpec<V> =
 ...
 * &nbsp;
 *
 * As such, it's recommended that [ArcAnimationSpec] is only used for positional 2D values, animated
 * through a multidimensional [TwoWayConverter].
 *
 * &nbsp;
 *
...
public value class StartOffsetType private constructor(
...
@kotlin.jvm.JvmInline
public value class StartOffset private constructor(
...
public class RepeatableSpec<T>(
    ...
    override fun hashCode(): Int =
        ((iterations * 31 + animation.hashCode()) * 31 + repeatMode.hashCode()) * 31 +
...
public class InfiniteRepeatableSpec<T>(
    ...
    init {
        ...
        val isZeroDuration =
            when (animation) {
                is TweenSpec -> {
                    animation.durationMillis == 0 && animation.delay == 0
                }
                ...
                is ArcAnimationSpec -> {
                    animation.durationMillis == 0 && animation.delayMillis == 0
                }
    ...
    override fun hashCode(): Int =
        (animation.hashCode() * 31 + repeatMode.hashCode()) * 31 +
            initialStartOffset.hashCode()
}
...
public sealed class KeyframesSpecBaseConfig<T, E : KeyframeBaseEntity<T>> {
    ...
    public open infix fun T.at(
        @IntRange(from = 0) timeStamp: Int,
    ): E {
     ...
     */
    // needed as `open` to guarantee binary compatibility in KeyframesSpecConfig
    public open infix fun T.atFraction(
        @FloatRange(from = 0.0, to = 1.0) fraction: Float,
    ): E = at((durationMillis * fraction).roundToInt())
...
public sealed class KeyframeBaseEntity<T>(
 ...
 */
@Immutable
public class KeyframesSpec<T>(
    public val config: KeyframesSpecConfig<T>,
) : DurationBasedAnimationSpec<T> {
    /**
    ...
    public class KeyframesSpecConfig<T> : KeyframesSpecBaseConfig<T, KeyframeEntity<T>>() {
        ...
        override infix fun T.atFraction(
            @FloatRange(from = 0.0, to = 1.0) fraction: Float,
        ): KeyframeEntity<T> = at((durationMillis * fraction).roundToInt())
         ...
         *
         * Note that arc modes are meant for objects with even dimensions (such as 2D positional values and their
 ...
 * it most to animate positional 2D values. For example:
 *
 *
...
public fun <T> keyframes(init: KeyframesSpecConfig<T>.() -> Unit): KeyframesSpec<T> =
    KeyframesSpec(KeyframesSpecConfig<T>().apply(init))
...
private class StartDelayVectorizedAnimationSpec<V : AnimationVector>(
    ...
    override fun hashCode(): Int = 31 * vectorizedAnimationSpec.hashCode() + startDelayNanos.hashCode()
...
private class StartDelayAnimationSpec<T>(
    ...
    override fun hashCode(): Int = 31 * animationSpec.hashCode() + startDelayNanos.hashCode()
