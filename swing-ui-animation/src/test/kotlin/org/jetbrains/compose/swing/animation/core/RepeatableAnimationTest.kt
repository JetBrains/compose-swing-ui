...
class RepeatableAnimationTest {
    ...
    fun twoRepeatsValuesCalculation() {
        ...
        assertEquals(0f, repeat.at(0))
        assertTrue(repeat.at(Duration - 1) > 0.9f)
        assertTrue(repeat.at(Duration + 1) < 0.1f)
        assertTrue(repeat.at(Duration * 2 - 1) > 0.9f)
        assertEquals(1f, repeat.at(Duration * 2))
        assertFalse(animationWrapper.isFinishedFromMillis(Duration * 2L - 1L))
        assertTrue(animationWrapper.isFinishedFromMillis(Duration * 2L))
    }
