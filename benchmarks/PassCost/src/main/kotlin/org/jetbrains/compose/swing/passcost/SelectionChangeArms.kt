 ...
 * on the list the first pass does not write. No selection is declared, so what a pass pays for is the
 * content alone.
 */
internal fun listItemsArm(): Arm =
    Arm(listOf(LIST_ITEMS_ARM)) { items, changing ->
        ...
        val declared = mutableStateOf(itemSets[1], referentialEqualityPolicy())
 ...
 * row to move to, so there the selection alternates between that row and none.
 */
internal fun listSelectionArm(): Arm =
    Arm(listOf(LIST_SELECTION_ARM)) { items, changing ->
        ...
        val declared = mutableStateOf(selections[1], referentialEqualityPolicy())
        val listRuns = IntArray(1)
        Run(
            content = { SelectingListBox(steadyItems, declared) { listRuns[0]++ } },
            drive = { pass ->
                if (changing) declared.value = selections[pass % 2]
                LIST_SELECTION_ARM
            },
            verify = { composed, passes ->
    ...
    Arm(listOf(TREE_SELECTION_ARM)) { nodes, changing ->
        ...
        Run(
            ...
            verify = { composed, passes ->
                ...
                val expected = selections[if (changing) (passes - 1) % 2 else 1]
                val shown = selectedChildIndices(tree)
                check(shown == expected) { "the tree has $shown selected, where $expected was declared last" }
            },
        )
    }
 ...
 */
private fun childPathSelections(nodes: Int): List<Set<List<Int>>> =
 ...
 * write the same declaration again - a re-write loop rather than the change the arm names.
 *
...
private fun tableSelectionArm(
    ...
    selections: (rows: Int) -> List<Set<Int>>,
): Arm =
    Arm(listOf(name)) { rows, changing ->
        val steadyRows = rowsOf(rows, STEADY_TEXT)
        val sets = selections(rows)
        val declared = mutableStateOf(sets[1], referentialEqualityPolicy())
        ...
        Run(
            ...
            drive = { pass ->
                if (changing) declared.value = sets[pass % 2]
                name
            },
            verify = { composed, passes ->
 ...
 */
private fun singleRowSelections(rows: Int): List<Set<Int>> =
    if (rows > 1) listOf(setOf(0), setOf(1)) else listOf(setOf(0), emptySet())

/** Two selections of one run of [SELECTED_INTERVAL_ROWS] rows each - the shape a single interval holds. */
private fun intervalSelections(rows: Int): List<Set<Int>> =
 ...
 */
private fun scatteredSelections(rows: Int): List<Set<Int>> =
